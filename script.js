...
function showScreen(id, el) {
  ...
  const screenPermMap = {
    ...
    'records': 'Records', 'voter-registry': 'Records', 'certificates': 'Certificates',
  ...
  if (id === 'voter-registry') loadVoterRegistry();
}
...
async function loadVoterRegistry() {
  VOTER_REGISTRY = await dbLoadVoterRegistry();
  renderVoterRegistry();
  updateVoterStats();
}

function renderVoterRegistry() {
  const tbody = document.getElementById('vr-tbody');
  const empty = document.getElementById('vr-empty');
  if (!tbody) return;

  let list = [...VOTER_REGISTRY];
  if (vrStatusFilter) list = list.filter(v => v.status === vrStatusFilter);
  if (vrSearch) {
    const q = vrSearch.toLowerCase();
    list = list.filter(v =>
      (v.resident_name || '').toLowerCase().includes(q) ||
      (v.resident_id || '').toLowerCase().includes(q) ||
      (v.purok || '').toLowerCase().includes(q) ||
  ...
  const statusColor = { Active: 'green', Transferred: 'amber', Deceased: 'red', Deactivated: 'gray' };

  tbody.innerHTML = list.map(v => {
    const color = statusColor[v.status] || 'gray';
    const regDate = v.registered_date ? v.registered_date.split('T')[0] : '—';
    return `<tr>
      <td><span style="font-family:var(--font-mono);font-size:11px;">${v.id}</span></td>
      <td><span style="font-family:var(--font-mono);font-size:11px;">${v.resident_id}</span></td>
      <td><strong>${v.resident_name || '—'}</strong><div style="font-size:11px;color:var(--text-muted);">${v.gender || ''} · ${v.age ? v.age + ' taong gulang' : ''}</div></td>
      <td>${v.purok || '—'}</td>
      <td>${v.precinct_no || '—'}</td>
      <td>${v.cluster_no || '—'}</td>
      <td>${regDate}</td>
      <td><span class="badge badge-${color}">${v.status}</span></td>
      <td>
        <button class="btn btn-ghost btn-sm" onclick="openEditVoterModal('${v.id}')">✏️ Edit</button>
        <button class="btn btn-ghost btn-sm" style="color:var(--red)" onclick="confirmRemoveVoter('${v.id}','${(v.resident_name||'').replace(/'/g,"\\'")}')">🗑️</button>
      </td>
    </tr>`;
  }).join('');
}
...
function updateVoterStats() {
  const total = VOTER_REGISTRY.length;
  const active = VOTER_REGISTRY.filter(v => v.status === 'Active').length;
  const transferred = VOTER_REGISTRY.filter(v => v.status === 'Transferred').length;

  const setEl = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
  setEl('vr-stat-total', total);
  setEl('vr-stat-active', active);
  setEl('vr-stat-transferred', transferred);

  // Unregistered = active residents not in voter registry
  if (typeof RESIDENTS !== 'undefined') {
    const voterResidentIds = new Set(VOTER_REGISTRY.map(v => v.resident_id));
    const unregistered = RESIDENTS.filter(r => r.status === 'Active' && !voterResidentIds.has(r.id)).length;
    setEl('vr-stat-unregistered', unregistered);
  }
}

function filterVoterRegistry() {
  vrSearch = (document.getElementById('vr-search')?.value || '').toLowerCase();
  renderVoterRegistry();
}

function filterVoterStatus(status, el) {
  vrStatusFilter = status;
  document.querySelectorAll('#screen-voter-registry .status-pill').forEach(p => p.classList.remove('active'));
  if (el) el.classList.add('active');
  renderVoterRegistry();
}

async function openRegisterVoterModal() {
  const select = document.getElementById('vr-reg-resident');
  if (!select) return;

  const unregistered = await dbGetUnregisteredResidents();
  select.innerHTML = '<option value="">— Pumili ng residente —</option>' +
    unregistered.map(r =>
      `<option value="${r.id}" data-name="${r.name}" data-purok="${r.purok||''}" data-age="${r.age||''}" data-gender="${r.gender||''}">${r.name} — ${r.purok||'N/A'}</option>`
    ).join('');

  ['vr-reg-precinct','vr-reg-cluster','vr-reg-verifiedby','vr-reg-remarks'].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = '';
  });
  const dateEl = document.getElementById('vr-reg-date');
  if (dateEl) dateEl.value = new Date().toISOString().split('T')[0];
  const infoEl = document.getElementById('vr-reg-resident-info');
  if (infoEl) infoEl.style.display = 'none';

  openModal('modal-register-voter');
}

function prefillVoterAge() {
  const select = document.getElementById('vr-reg-resident');
  const infoEl = document.getElementById('vr-reg-resident-info');
  ...
  infoEl.innerHTML = `<strong>${opt.dataset.name}</strong> &nbsp;·&nbsp; ${opt.dataset.gender || ''} &nbsp;·&nbsp; ${opt.dataset.age ? opt.dataset.age + ' taong gulang' : ''} &nbsp;·&nbsp; Purok: ${opt.dataset.purok || 'N/A'}`;
}

async function submitRegisterVoter() {
  const residentId = document.getElementById('vr-reg-resident')?.value;
  if (!residentId) { showToast('Pumili muna ng residente.', 'red'); return; }

  const payload = {
    resident_id:      residentId,
    precinct_no:      document.getElementById('vr-reg-precinct')?.value || '',
    cluster_no:       document.getElementById('vr-reg-cluster')?.value || '',
    registered_date:  document.getElementById('vr-reg-date')?.value || '',
    verified_by:      document.getElementById('vr-reg-verifiedby')?.value || '',
    remarks:          document.getElementById('vr-reg-remarks')?.value || '',
    status:           'Active',
  };

  const result = await dbRegisterVoter(payload);
  if (result && result.ok) {
    closeModal('modal-register-voter');
    showToast('✅ Matagumpay na narehistro ang botante!', 'green');
    await loadVoterRegistry();
  } else {
    const msg = result?.error || 'Hindi mairehistro. Subukang muli.';
    showToast('❌ ' + msg, 'red');
  }
}

function openEditVoterModal(vid) {
  const voter = VOTER_REGISTRY.find(v => v.id === vid);
  if (!voter) return;

  document.getElementById('vr-edit-id').value = voter.id;
  document.getElementById('vr-edit-name').value = voter.resident_name || '';
  document.getElementById('vr-edit-precinct').value = voter.precinct_no || '';
  document.getElementById('vr-edit-cluster').value = voter.cluster_no || '';
  document.getElementById('vr-edit-date').value = voter.registered_date ? voter.registered_date.split('T')[0] : '';
  document.getElementById('vr-edit-status').value = voter.status || 'Active';
  document.getElementById('vr-edit-verifiedby').value = voter.verified_by || '';
  document.getElementById('vr-edit-remarks').value = voter.remarks || '';

  openModal('modal-edit-voter');
}

async function submitEditVoter() {
  const vid = document.getElementById('vr-edit-id')?.value;
  if (!vid) return;

  const payload = {
    precinct_no:     document.getElementById('vr-edit-precinct')?.value || '',
    cluster_no:      document.getElementById('vr-edit-cluster')?.value || '',
    registered_date: document.getElementById('vr-edit-date')?.value || '',
    status:          document.getElementById('vr-edit-status')?.value || 'Active',
    verified_by:     document.getElementById('vr-edit-verifiedby')?.value || '',
    remarks:         document.getElementById('vr-edit-remarks')?.value || '',
  };

  const result = await dbUpdateVoter(vid, payload);
  if (result && result.ok) {
    closeModal('modal-edit-voter');
    showToast('✅ Na-update ang voter record.', 'green');
    await loadVoterRegistry();
  } else {
    showToast('❌ Hindi ma-update. Subukang muli.', 'red');
  }
}

async function confirmRemoveVoter(vid, name) {
  if (!confirm(`Tanggalin si "${name}" sa Voter Registry? Hindi na ito mababawi.`)) return;
  const result = await dbRemoveVoter(vid);
  if (result && result.ok) {
    showToast('🗑️ Voter record natanggal.', 'green');
    await loadVoterRegistry();
  } else {
    showToast('❌ Hindi matanggal. Subukang muli.', 'red');
  }
}

function exportVoterList() {
  if (!VOTER_REGISTRY.length) { showToast('Walang datos para i-export.', 'red'); return; }
  const rows = [['Voter ID','Resident ID','Pangalan','Purok','Precinct No.','Cluster No.','Petsa ng Pagpaparehistro','Status','Remarks']];
  VOTER_REGISTRY.forEach(v => {
    rows.push([v.id, v.resident_id, v.resident_name||'', v.purok||'', v.precinct_no||'', v.cluster_no||'', v.registered_date||'', v.status, v.remarks||'']);
  });
  const csv = rows.map(r => r.map(c => `"${String(c).replace(/"/g,'""')}"`).join(',')).join('\n');
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }));
  a.download = 'voter-registry-anabu1g.csv';
  a.click();
  showToast('✅ Na-export ang voter list.', 'green');
}
