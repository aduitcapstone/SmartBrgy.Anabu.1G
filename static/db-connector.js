...
// ── Voter Registry ────────────────────────────────────────────
async function dbLoadVoterRegistry() {
  const data = await http.get('/api/voters');
  return Array.isArray(data) ? data : [];
}

async function dbGetUnregisteredResidents() {
  const data = await http.get('/api/voters/unregistered');
  return Array.isArray(data) ? data : [];
}

async function dbRegisterVoter(payload) {
  return await http.post('/api/voters', payload);
}

async function dbUpdateVoter(vid, payload) {
  return await http.put(`/api/voters/${vid}`, payload);
}

async function dbRemoveVoter(vid) {
  return await http.del(`/api/voters/${vid}`);
}
