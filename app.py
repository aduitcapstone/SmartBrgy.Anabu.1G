    ...
    try:
        with conn.cursor() as cur:
            ...
            CREATE TABLE IF NOT EXISTS voter_registry (
                ...
                updated_at       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP
                                 ON UPDATE CURRENT_TIMESTAMP,
                UNIQUE KEY uq_vr_resident (resident_id),
                FOREIGN KEY (resident_id) REFERENCES residents(id)
                    ON DELETE CASCADE ON UPDATE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
            """)
...
def get_voters():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT v.*, r.name AS resident_name, r.purok, r.dob, r.gender, r.status AS resident_status
                FROM voter_registry v
        ...
        result = []
        for row in rows:
            for k in ('registered_date', 'verified_at', 'created_at', 'updated_at'):
                if isinstance(row.get(k), (datetime, date)):
                    row[k] = str(row[k])
            row['age'] = calc_age(str(row.get('dob') or ''))
            if isinstance(row.get('dob'), (datetime, date)):
                row['dob'] = str(row['dob'])
            result.append(row)
        return jsonify(result)
    finally:
...
@app.route('/api/voters/unregistered', methods=['GET'])
def get_unregistered_residents():
    """Returns residents not yet in voter_registry — para sa dropdown ng register modal."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT r.id, r.name, r.purok, r.dob, r.gender
                FROM residents r
                WHERE r.id NOT IN (SELECT resident_id FROM voter_registry)
                  AND r.status = 'Active'
                ORDER BY r.name
            """)
            rows = cur.fetchall()
...
def register_voter():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT id, name FROM residents WHERE id=%s", (resident_id,))
            resident = cur.fetchone()
            if not resident:
                return jsonify({'error': 'Resident not found'}), 404
            cur.execute("SELECT id FROM voter_registry WHERE resident_id=%s", (resident_id,))
            if cur.fetchone():
                return jsonify({'error': 'Resident is already a registered voter'}), 409
            vid = 'VTR-' + str(uuid.uuid4())[:8].upper()
            cur.execute("""
                INSERT INTO voter_registry
                    (id, resident_id, precinct_no, cluster_no, registered_date, status, verified_by, remarks)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                vid,
                resident_id,
                data.get('precinct_no', ''),
                data.get('cluster_no', ''),
                data.get('registered_date') or None,
                data.get('status', 'Active'),
                data.get('verified_by', ''),
                data.get('remarks', '')
            ))
            add_audit(cur, '🗳️', 'voters', 'Voter Registered',
                      f"{resident['name']} ({resident_id}) — Precinct {data.get('precinct_no','N/A')}",
                      data.get('verified_by', 'Staff'))
            conn.commit()
        return jsonify({'ok': True, 'id': vid}), 201
    finally:
        conn.close()

@app.route('/api/voters/<vid>', methods=['PUT'])
def update_voter(vid):
    data = request.json or {}
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT v.id, r.name FROM voter_registry v JOIN residents r ON v.resident_id=r.id WHERE v.id=%s", (vid,))
            row = cur.fetchone()
            if not row:
                return jsonify({'error': 'Not found'}), 404
            cur.execute("""
                UPDATE voter_registry
                SET precinct_no=%s, cluster_no=%s, registered_date=%s,
                    status=%s, verified_by=%s, verified_at=%s, remarks=%s
                WHERE id=%s
            """, (
                data.get('precinct_no', ''),
                data.get('cluster_no', ''),
                ...
                data.get('status', 'Active'),
                data.get('verified_by', ''),
                datetime.now().strftime('%Y-%m-%d %H:%M:%S') if data.get('verified_by') else None,
                data.get('remarks', ''),
                vid
            ))
            add_audit(cur, '✏️', 'voters', 'Voter Record Updated',
                      f"{row['name']} ({vid})", data.get('verified_by', 'Staff'))
            conn.commit()
        return jsonify({'ok': True})
    finally:
        conn.close()

@app.route('/api/voters/<vid>', methods=['DELETE'])
def delete_voter(vid):
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not row:
                return jsonify({'error': 'Not found'}), 404
            cur.execute("DELETE FROM voter_registry WHERE id=%s", (vid,))
            add_audit(cur, '🗑️', 'voters', 'Voter Record Removed', f"{row['name']} ({vid})", 'Staff')
            conn.commit()
        return jsonify({'ok': True})
    finally:
        conn.close()
